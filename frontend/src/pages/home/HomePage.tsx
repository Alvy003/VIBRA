...
import { Play, Sparkles, TrendingUp, Music2, Disc3, ChevronLeft, ChevronRight } from "lucide-react";
...
const HeroBackground = ({ imageUrl, isLoaded }: { imageUrl: string; isLoaded: boolean }) => (
  <div className="absolute inset-0 overflow-hidden">
    ...
    {/* Multi-layer gradient for seamless blend */}
...
// utils — shuffle once (Fisher–Yates)
...
// Featured Song Card - More prominent design
const FeaturedSongPill = ({ song, onClick }: { song: Song; onClick: () => void }) => (
  <motion.button
    onClick={onClick}
    whileHover={{ scale: 1.02 }}
    whileTap={{ scale: 0.98 }}
    className="flex items-center gap-3 bg-white/10 backdrop-blur-md hover:bg-white/20 active:hover:bg-white/20 rounded-full pl-1 pr-4 py-1 transition-all border border-white/10 group"
  >
    <img
      src={song.imageUrl}
      alt={song.title}
    ...
    <div className="flex-1 min-w-0 pr-2">
      <p
        className={cn(
          ...
          !isTouchDevice &&
            "max-w-full group-hover:max-w-[calc(100%-2.5rem)]"
        )}
      >
        {song.title}
      </p>
...
// Horizontal Rail Component for Desktop (Spotify-style)
      ...
      <AnimatePresence>
        {canScrollLeft && (
          <motion.button
            ...
            exit={{ opacity: 0 }}
            onClick={() => scroll('left')}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-zinc-900/90 hover:bg-zinc-800 hover:scale-105 flex items-center justify-center shadow-lg opacity-0 group-hover/rail:opacity-100 transition-all duration-200"
          >
      ...
      <AnimatePresence>
        {canScrollRight && (
          <motion.button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-zinc-900/90 hover:bg-zinc-800 hover:scale-105 flex items-center justify-center shadow-lg opacity-0 group-hover/rail:opacity-100 transition-all duration-200"
          >
...
// In HomePage.tsx - Updated AlbumCard
const AlbumCard = ({
  album,
  isTouchDevice,
}: {
...
const HomePage = () => {
  ...
  const randomAlbums = useMemo(() => {
    ...
    // Use a stable seed based on session (won't change on re-render)
    ...
    // Seeded shuffle for consistency within session
